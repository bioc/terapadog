...
- *Rank*. This corresponds to the Value of S80 in the original PADOG paper. It's
the ranking (based on enrichment) for a pathway when compared to all others.
- *Ppadog*. This is the p-value corresponding to the PADOG statistic (padog0), 
calculated via permutation testing.

**NB.** RESULTS INTERPRETATION ADVICE
A current limit of PADOG is that the PPADOG has a floor level determined by the 
number of iterations (which is in turn determined by how many samples you have).
RNA-seq experiments (or Ribo-Seq as well) tends to not have so many replicates.
As such, PPADOG might be coarse as a p-value. 
*As an heuristic for result interpretation: start by looking at pathways that* 
*have significant PPADOG AND highest (from 1 ascending) rank.*

```{r}
# Load the example of terapadog results from the library
example <- system.file("extdata", "terapadog_res_example.csv", 
                                    package = "terapadog")
res_example <- read.table(example, sep = ",", header = TRUE)
...
*results of the terapadog() function (eg. 04350).*
...
# Execute the function
pathway_res <- get_gene_from_kegg_pathway(fc_results, "04350")
```
...
# Execute the function
plots <- plot_pathway_changes(fc_results, "04350")
```
...
For the single-pathway visualization of the results, terapadog imports pathview, 
which we reference below.

Luo, Weijun, Brouwer, Cory. Pathview: an R/Bioconductor package for pathway-based data integration and visualization. Bioinformatics, 29(14), 1830-1831 (2013).

[<https://doi:10.1093/bioinformatics/btt285>]

*ReactomeGSA:*

Finally, I reference the ReactomeGSA, since this tool has been developed with the 
ultimate goal to have it integrated with it, as part of a new analyser.
